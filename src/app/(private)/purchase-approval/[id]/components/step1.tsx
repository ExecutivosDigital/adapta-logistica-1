        ...
        <div className="flex w-full flex-row items-center justify-center">
          <div className="mt-2 flex gap-2">
            <div className="bg-primary/40 relative flex w-96 flex-row overflow-hidden rounded-lg p-2">
              <div
                className={`absolute top-0 bottom-0 left-0 flex w-1/2 transform items-center justify-center transition-transform duration-300 ${data.entryType === "DESPESAS" ? "translate-x-0 pl-2" : data.entryType === "IMPOSTOS" ? "translate-x-full" : "translate-x-full pr-2"}`}
              >
                <div className="bg-primary h-[80%] w-[95%] rounded-lg"></div>
              </div>
              <button
                onClick={() => setData({ ...data, entryType: "DESPESAS" })}
                className={`relative z-10 w-1/2 px-4 py-1 text-sm transition-all duration-300 ${data.entryType === "DESPESAS" ? "font-semibold text-white" : "text-white/80"}`}
              >
                DESPESAS
              </button>
              <button
                onClick={() => setData({ ...data, entryType: "C. VENDAS" })}
                className={`relative z-10 w-1/2 px-4 py-1 text-sm transition-all duration-300 ${data.entryType === "C. VENDAS" ? "font-semibold text-white" : "text-white/80"}`}
              >
              ...
              <div className="relative flex h-12 items-center gap-2 rounded-2xl border border-zinc-200 px-2 py-1 xl:h-16 xl:px-3 xl:py-2">
                ...
                <div className="flex h-full flex-1 items-center justify-center text-center">
                  <input
                    ...
                    placeholder="R$ 0,00"
                    className="w-full flex-1 items-center bg-transparent text-center text-zinc-700 outline-none 2xl:text-lg"
                  />
                </div>
