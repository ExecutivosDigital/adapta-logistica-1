...
export function SuppliersAndCustomersTable() {
  ...
  /* ------------------------------ Tab & Rows ------------------------------- */
  const allRows = useMemo(() => [...toPay, ...toReceive], []);
  const rawRows: TransactionProps[] = useMemo(() => {
    switch (selectedTableType.name as string) {
      case "Receber":
        return toReceive;
      case "Pagar":
        return toPay;
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex w-full flex-col items-center justify-between xl:flex-row">
        ...
        <div className="flex min-w-80 items-center justify-between gap-2 rounded-md border border-zinc-400 px-2 py-1">
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Pesquisar"
            className="w-full text-sm placeholder:text-zinc-400"
          />
